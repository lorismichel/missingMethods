...
require(torus)
n<-500
d <- genData(n =n , dataset = "parabola", pNA = 0)
...
et <- extraTorus(X = d$X.NA, nb.nodes = 12)
...
w_ct_all<-getSampleWeights(ct, X = rbind(X.NA.grid,d$X.NA),
                       prob = c(0.005,0.005,1-2*0.005), d="euclidean", method="eigen")
...
par(mfrow=c(1,1))
plot(d$X.NA)
points(d$X.NA[order(w_ct_all[6,7:(n+5)], decreasing = FALSE)[1:round(n/10)],], col="green")
points(d$X.NA[1,1],d$X.NA[1,2], col="red" )
}
